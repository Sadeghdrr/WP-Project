...
--------
  ...
  6.8  Sergeant review after suspects declared
         A  POST /cases/{id}/sergeant-review/ decision="approve" → 200; status == "arrest_ordered"
         A2 Status log contains "arrest_ordered" entry
         B  POST /cases/{id}/sergeant-review/ decision="reject" + message → 200; status == "investigation"
         B2 Rejection message stored in status log entry
         C  Non-sergeant (assigned detective) tries sergeant-review → 403
         D  Reject without message → 400 (serializer validation)
         E  Sergeant-review on case NOT in SERGEANT_REVIEW → 409 Conflict
"""
...
class TestBoardAndSuspectsFlow(TestCase):
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Scenario 6.8 — Sergeant review after suspects declared
    # ─────────────────────────────────────────────────────────────────────

    # ── helpers ───────────────────────────────────────────────────────

    def _setup_sergeant_review_case(self) -> int:
        """
        Arrange a case in SERGEANT_REVIEW with sergeant_user as the
        assigned sergeant, by extending _setup_investigation_case:

          1. _setup_investigation_case()
               – grants CAN_ASSIGN_DETECTIVE to sergeant_role
               – grants CAN_CHANGE_CASE_STATUS to detective_role
               – creates OPEN case, Sergeant assigns Detective → INVESTIGATION

          2. Grant CAN_CHANGE_CASE_STATUS to sergeant_role
             (needed for SERGEANT_REVIEW → ARREST_ORDERED / INVESTIGATION
              inside transition_state, since requesting_user is the sergeant).

          3. Sergeant assigns themselves via
             POST /api/cases/{id}/assign-sergeant/ (uses CAN_ASSIGN_DETECTIVE).

          4. Detective calls POST /api/cases/{id}/declare-suspects/
             (INVESTIGATION → SUSPECT_IDENTIFIED → SERGEANT_REVIEW).
        ...
        # Grant sergeant the permission needed to perform transitions
        _assign_permission_to_role(self.sergeant_role, "can_change_case_status", "cases")

        # Sergeant assigns themselves to the case
        self._login_as(self.sergeant_user.username, self.sergeant_password)
        assign_url = reverse("case-assign-sergeant", kwargs={"pk": case_id})
        assign_resp = self.client.post(
            assign_url,
            {"user_id": self.sergeant_user.pk},
            format="json",
        )
        self.assertEqual(
            assign_resp.status_code,
            status.HTTP_200_OK,
            msg=f"assign-sergeant failed in _setup_sergeant_review_case: {assign_resp.data}",
        )

        # Detective declares suspects: INVESTIGATION → SERGEANT_REVIEW
        self._login_as(self.detective_user.username, self.detective_password)
        declare_resp = self._declare_suspects(case_id)
        self.assertEqual(
            declare_resp.status_code,
            status.HTTP_200_OK,
            msg=f"declare-suspects failed in _setup_sergeant_review_case: {declare_resp.data}",
        )
        self.assertEqual(
            declare_resp.data.get("status"),
            "sergeant_review",
            msg="Case should be SERGEANT_REVIEW after declare-suspects.",
        )
        return case_id

    def _sergeant_review(
        self, case_id: int, decision: str, message: str = ""
    ) -> object:
        """
        POST /api/cases/{id}/sergeant-review/
        Payload: {"decision": <"approve"|"reject">, "message": <str>}
        """
        url = reverse("case-sergeant-review", kwargs={"pk": case_id})
        payload: dict = {"decision": decision}
        if message:
            payload["message"] = message
        return self.client.post(url, payload, format="json")

    # ── 6.8-A: sergeant approves → 200 + arrest_ordered ───────────────────

    def test_6_8_a_sergeant_approves_returns_200(self) -> None:
        """
        Scenario 6.8-A: POST /api/cases/{id}/sergeant-review/ with
        decision="approve" as the assigned sergeant returns HTTP 200.

        Service: CaseWorkflowService.process_sergeant_review
          — asserts case.status == SERGEANT_REVIEW
          — asserts requesting_user == case.assigned_sergeant
          — transitions SERGEANT_REVIEW → ARREST_ORDERED

        Reference: cases/services.py — process_sergeant_review
        """
        case_id = self._setup_sergeant_review_case()
        self._login_as(self.sergeant_user.username, self.sergeant_password)

        response = self._sergeant_review(case_id, decision="approve")

        self.assertEqual(
            response.status_code,
            status.HTTP_200_OK,
            msg=(
                f"Expected 200 OK when sergeant approves, "
                f"got {response.status_code}: {response.data}"
            ),
        )

    def test_6_8_a2_approve_transitions_to_arrest_ordered(self) -> None:
        """
        Scenario 6.8-A2 (status check): Response body status must be
        "arrest_ordered" after sergeant approves.

        Reference: cases/models.py — CaseStatus.ARREST_ORDERED = "arrest_ordered"
        """
        case_id = self._setup_sergeant_review_case()
        self._login_as(self.sergeant_user.username, self.sergeant_password)
        ...
        self.assertEqual(
            ...
            "arrest_ordered",
            msg=(
                f"Expected status='arrest_ordered' after approval, "
                f"got: {response.data.get('status')!r}"
            ),
        )

        # Confirm status log contains arrest_ordered entry
        detail = self._get_case(case_id)
        to_statuses = [log["to_status"] for log in detail.data.get("status_logs", [])]
        self.assertIn(
            "arrest_ordered",
            to_statuses,
            msg="Expected CaseStatusLog entry with to_status='arrest_ordered'.",
        )

    # ── 6.8-B: sergeant rejects → 200 + back to investigation ────────────

    def test_6_8_b_sergeant_rejects_returns_200(self) -> None:
        """
        Scenario 6.8-B: POST /api/cases/{id}/sergeant-review/ with
        decision="reject" and a non-blank message returns HTTP 200.

        Service: CaseWorkflowService.process_sergeant_review
          — transitions SERGEANT_REVIEW → INVESTIGATION
          — stores rejection message in CaseStatusLog
        ...
        response = self._sergeant_review(
            case_id, decision="reject", message="Insufficient evidence against suspect #2."
        )
        ...
        self.assertEqual(
            ...
            status.HTTP_200_OK,
            msg=(
                f"Expected 200 OK when sergeant rejects, "
                f"got {response.status_code}: {response.data}"
            ),
        )

    def test_6_8_b2_reject_status_is_investigation_and_message_stored(self) -> None:
        """
        Scenario 6.8-B2 (persistence check):
          • Response status must be "investigation" (case returned to detective).
          • A CaseStatusLog entry with to_status='investigation' and the
            rejection message must exist in GET /api/cases/{id}/.

        Reference: cases/services.py — transition_state (step 6: CaseStatusLog)
        """
        case_id = self._setup_sergeant_review_case()
        self._login_as(self.sergeant_user.username, self.sergeant_password)
        rejection_msg = "Insufficient evidence against suspect #2."

        response = self._sergeant_review(
            case_id, decision="reject", message=rejection_msg
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            ...
            "investigation",
            msg=(
                f"Expected status='investigation' after rejection, "
                f"got: {response.data.get('status')!r}"
            ),
        )

        # Confirm message stored in status log
        self._login_as(self.detective_user.username, self.detective_password)
        detail = self._get_case(case_id)
        self.assertEqual(detail.status_code, status.HTTP_200_OK)

        rejection_logs = [
            log for log in detail.data.get("status_logs", [])
            if log.get("to_status") == "investigation"
            and log.get("message") == rejection_msg
        ]
        self.assertTrue(
            len(rejection_logs) > 0,
            msg=(
                f"Expected a CaseStatusLog with to_status='investigation' and "
                f"message='{rejection_msg}', but none found. "
                f"Logs: {detail.data.get('status_logs', [])}"
            ),
        )

    # ── 6.8-C: non-sergeant tries sergeant-review → 403 ────────────────

    def test_6_8_c_non_sergeant_cannot_perform_sergeant_review_returns_403(self) -> None:
        """
        Scenario 6.8-C: The assigned detective (not the assigned sergeant)
        attempts POST /api/cases/{id}/sergeant-review/ and is rejected with
        HTTP 403.

        Service guard:
            CaseWorkflowService.process_sergeant_review
            → PermissionDenied("Only the assigned sergeant can perform this review.")
              when requesting_user != case.assigned_sergeant

        Reference: cases/services.py — process_sergeant_review, line ~1110
        """
        case_id = self._setup_sergeant_review_case()

        # Detective (the assigned detective, not the sergeant) attempts review
        self._login_as(self.detective_user.username, self.detective_password)
        response = self._sergeant_review(case_id, decision="approve")
        ...
        self.assertEqual(
            ...
            status.HTTP_403_FORBIDDEN,
            msg=(
                f"Expected 403 Forbidden when non-sergeant performs sergeant review, "
                f"got {response.status_code}: {response.data}"
            ),
        )

    # ── 6.8-D: reject without message → 400 ───────────────────────────

    def test_6_8_d_reject_without_message_returns_400(self) -> None:
        """
        Scenario 6.8-D: POST /api/cases/{id}/sergeant-review/ with
        decision="reject" but an empty message returns HTTP 400, because
        the serializer's cross-field validator enforces a non-blank
        rejection reason.

        Serializer guard:
            SergeantReviewSerializer.validate()
            → ValidationError({"message": "A rejection reason is required."})
              when decision == "reject" and message is blank/missing.

        Reference: cases/serializers.py — SergeantReviewSerializer.validate
        """
        case_id = self._setup_sergeant_review_case()
        self._login_as(self.sergeant_user.username, self.sergeant_password)
        ...
        url = reverse("case-sergeant-review", kwargs={"pk": case_id})
        response = self.client.post(
            url, {"decision": "reject", "message": ""}, format="json"
        )
        ...
        self.assertEqual(
            ...
            status.HTTP_400_BAD_REQUEST,
            msg=(
                f"Expected 400 Bad Request for reject without message, "
                f"got {response.status_code}: {response.data}"
            ),
        )

    # ── 6.8-E: wrong case status → 409 ────────────────────────────────

    def test_6_8_e_sergeant_review_on_non_review_case_returns_409(self) -> None:
        """
        Scenario 6.8-E: Calling POST /api/cases/{id}/sergeant-review/ on a
        case that is NOT in SERGEANT_REVIEW status returns HTTP 409.
        ...
        Service guard:
            ...
            → view maps InvalidTransition → 409 Conflict

        Reference: cases/services.py — process_sergeant_review, line ~1103
        """
        # Need a case in INVESTIGATION (not SERGEANT_REVIEW) with sergeant assigned
        _assign_permission_to_role(self.sergeant_role, "can_assign_detective", "cases")
        _assign_permission_to_role(self.sergeant_role, "can_change_case_status", "cases")
        _assign_permission_to_role(self.detective_role, "can_change_case_status", "cases")

        invest_case = Case.objects.create(
            title="Wrong Status Case — Scenario 6.8-E",
        ...
        self._login_as(self.sergeant_user.username, self.sergeant_password)
        response = self._sergeant_review(invest_case.pk, decision="approve")

        self.assertEqual(
            response.status_code,
            status.HTTP_409_CONFLICT,
            msg=(
                f"Expected 409 Conflict when case is not in SERGEANT_REVIEW, "
                f"got {response.status_code}: {response.data}"
            ),
        )
